...
### Obtaining population-specific variants data

All vairants (SNPs, indels) for individuals from the AFR, EAS and EUR populations

```
bcftools view All.chr.vcf.gz -S afr.txt -O z -o All.chr.afr.vcf.gz
bcftools view All.chr.vcf.gz -S asn.txt -O z -o All.chr.asn.vcf.gz
...
```
bcftools filter -i 'TYPE="snp" && MAF[0]>0.05' All.chr.afr.vcf.gz
bcftools filter    All.chr.asn.vcf.gz
bcftools filter    All.chr.eur.vcf.gz
```
...
```
plink2 --vcf All.chr.afr.vcf.gz --maf 0.05 --recode --make-bed --out ./plink_assoc/All.chr.afr
```
...
```
plink2 --bfile All.chr.afr --pheno ../polyTE_genotype_AFR.txt --all-pheno  --assoc --allow-no-sex --pfilter 1e-3 --out All.chr.afr
```
...
assocTEloc = assocTEloc[match(assocTEnames, assocTEloc$ID), ]
