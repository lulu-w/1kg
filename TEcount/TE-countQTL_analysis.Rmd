...
## `r nrow(sample_info)` individuals from 1KG project

```{r}
counts = unlist(table(paste0(sample_info$group, "_", sample_info$population)))
labels = gsub("^[A-Z]+_", "", names(counts))
cg5 = factor(gsub("_[A-Z]+$", "", names(counts)), labels = c5)


par(cex = 1, las = 2)
xpos = barplot(counts, ylim = c(0, 140), xlab = "Population", ylab = "Number of individuals", names.arg = labels, col = as.character(cg5), space=c(2, rep(1, 4), 2, rep(1, 5), 2, rep(1, 4), 2, rep(1, 4), 2, rep(1, 4)))
text(xpos, counts, labels = counts, pos = 3)

plot.new()
legend("center", names(sample_info_pop), fill = c5, bty = "n", border = NA)
#legend("topright", names(sample_info_pop), fill = c5, horiz = T, bty = "n", border = NA, cex = 0.7)
```
...
### GWAS analysis using Matrix eQTL

1. Generating population-specific variant file for AFR, EAS, EUR

```
bcftools view All.chr.vcf.gz -S afr.txt -O z -o All.chr.afr.vcf.gz
bcftools view All.chr.vcf.gz -S eas.txt -O z -o All.chr.eas.vcf.gz
bcftools view All.chr.vcf.gz -S eur.txt -O z -o All.chr.eur.vcf.gz
```

2. Filter variant by MAF and variant type

SNPs only data set with MAF > 0.05

```
bcftools filter -i 'TYPE="snp" && MAF[0]>0.05' All.chr.afr.vcf.gz -O z -o afr.snp.maf05.vcf.gz
bcftools filter -i 'TYPE="snp" && MAF[0]>0.05' All.chr.eas.vcf.gz -O z -o eas.snp.maf05.vcf.gz
bcftools filter -i 'TYPE="snp" && MAF[0]>0.05' All.chr.eur.vcf.gz -O z -o eur.snp.maf05.vcf.gz
```

3. Generating input **genotype** and **snploc** files for Matrix eQTL

```
# AFR
bcftools view afr.snp.maf05.vcf.gz | grep -v '^##' | awk '{ count=gsub(",", " ", $5) } { if( count==0 ) print $0 }' | tee >(grep -v '^#' | ((printf "snp\tchr\tpos\n") && (awk 'BEGIN{OFS="\t"}{print $3,"chr"$1,$2}')) > snploc.afr) | cut -f3,10- | sed 's/0|0/0/g' | sed 's/\(0|1\)\|\(1|0\)/1/g' | sed 's/1|1/2/g'> snps.afr

# EAS
bcftools view eas.snp.maf05.vcf.gz | grep -v '^##' | awk '{ count=gsub(",", " ", $5) } { if( count==0 ) print $0 }' | tee >(grep -v '^#' | ((printf "snp\tchr\tpos\n") && (awk 'BEGIN{OFS="\t"}{print $3,"chr"$1,$2}')) > snploc.eas) | cut -f3,10- | sed 's/0|0/0/g' | sed 's/\(0|1\)\|\(1|0\)/1/g' | sed 's/1|1/2/g'> snps.eas

# EUR
bcftools view eur.snp.maf05.vcf.gz | grep -v '^##' | awk '{ count=gsub(",", " ", $5) } { if( count==0 ) print $0 }' | tee >(grep -v '^#' | ((printf "snp\tchr\tpos\n") && (awk 'BEGIN{OFS="\t"}{print $3,"chr"$1,$2}')) > snploc.eur) | cut -f3,10- | sed 's/0|0/0/g' | sed 's/\(0|1\)\|\(1|0\)/1/g' | sed 's/1|1/2/g'> snps.eur

```

4. Run Matrix eQTL association analysis



```{r}
# The geneExp_AFR.txt, geneExp_EAS.txt, geneExp_EUR.txt, covariates_AFR.txt..ect. are matrices of TE counts sorted (columns) by individual ID while snps.afr, snps.eas, snps.eur are not sorted. Therefore, sort snps.afr, snps.eas and snps.eur by ID before running Matrix eQTL association analysis.

# require(data.table)
# # AFR
# temp = fread("snps.afr", header = T)
# o = order(names(temp)[2:ncol(temp)])
# o = c(1, o+1)
# temp = temp[, o, with = F]
# write.table(temp, "snps.afr.sorted", quote = F, row.names = F, sep = "\t")
# 
# # EAS
# temp = fread("snps.eas", header = T)
# o = order(names(temp)[2:ncol(temp)])
# o = c(1, o+1)
# temp = temp[, o, with = F]
# write.table(temp, "snps.eas.sorted", quote = F, row.names = F, sep = "\t")
# 
# # EUR
# temp = fread("snps.eur", header = T)
# o = order(names(temp)[2:ncol(temp)])
# o = c(1, o+1)
# temp = temp[, o, with = F]
# write.table(temp, "snps.eur.sorted", quote = F, row.names = F, sep = "\t")
```



```{r eval = F}
# # Run this chunk on abilbeast ONLY
# # Run this chunck under abilbeast/......../bcf_concat
# # source("../functions.R"); options(stringsAsFactors = FALSE)
# 
# # Global par
# 
# p.cut = 1e-2
...
# 
# # AFR
# SNP_file_name = "snps.afr.sorted"
# snps_location_file_name = "snploc.afr"
# expression_file_name = "../temp/geneExp_AFR.txt"
# gene_location_file_name = "../abilbeast_input/presenceLoc.txt"
# covariates_file_name = "../temp/covariates_AFR.txt"
# 
# 
...
#      noFDRsaveMemory = F)
# popme[['AFR']] = res$all$eqtls
# pop_snploc[['AFR']] = read.table(snps_location_file_name, header = T, stringsAsFactors = F)
# pop_snploc[['AFR']] = pop_snploc[['AFR']][!duplicated(pop_snploc[['AFR']]$snp), ]
# 
# 
# # EAS
# SNP_file_name = "snps.eas.sorted"
# snps_location_file_name = "snploc.eas"
# expression_file_name = "../temp/geneExp_EAS.txt"
# gene_location_file_name = "../abilbt_input/presenceLoc.txt"
# covariates_file_name = "../temp/covariates_EAS.txt"
# 
# 
# res = runME( SNP_file_name, 
#      snps_location_file_name, 
...
#      pvOutputThreshold = p.cut, 
#      noFDRsaveMemory = F)
# popme[['EAS']] = res$all$eqtls
# pop_snploc[['EAS']] = read.table(snps_location_file_name, header = T, stringsAsFactors = F)
# pop_snploc[['EAS']] = pop_snploc[['EAS']][!duplicated(pop_snploc[['EAS']]$snp), ]
# 
# 
# # EUR
# SNP_file_name = "snps.eur.sorted"
# snps_location_file_name = "snploc.eur"
# expression_file_name = "../temp/geneExp_EUR.txt"
# gene_location_file_name = "../abilbt_input/presenceLoc.txt"
# covariates_file_name = "../temp/covariates_EUR.txt"
# 
# 
# res = runME( SNP_file_name, 
#      snps_location_file_name, 
...
#      pvOutputThreshold = p.cut, 
#      noFDRsaveMemory = F)
# popme[['EUR']] = res$all$eqtls
# pop_snploc[['EUR']] = read.table(snps_location_file_name, header = T, stringsAsFactors = F)
# pop_snploc[['EUR']] = pop_snploc[['EUR']][!duplicated(pop_snploc[['EUR']]$snp), ]
# 
# 
#     
# # Save Manhattan plot pvalues
# require(dplyr)    
...
#        file = paste0("ChrAll", "_", names(popme_pval), ".txt"), 
...
#            file = paste0("AllPvalues_", names(popme), ".txt"), 
#            quote = rep(FALSE, length(popme)), 
#            row.names = rep(FALSE, length(popme)), 
#            sep = rep("\t", length(popme)) )
```
...
myQQplot("./analysis_input/AllPvalues_AFR.txt", "./analysis_input/chrAllPvalues_AFR_Permuted_pval.txt", n_probs=100000, p_cut=0.01, col=c5[1], title=names(c5)[1])


# EAS

myQQplot("./analysis_input/AllPvalues_EAS.txt", "./analysis_input/chrAllPvalues_EAS_Permuted_pval.txt", n_probs=100000, p_cut=0.01, col=c5[3], title=paste(names(c5)[3]))

# EUR

myQQplot("./analysis_input/AllPvalues_AFR.txt", "./analysis_input/chrAllPvalues_EUR_Permuted_pval.txt", n_probs=100000, p_cut=0.01, col=c5[4], title=names(c5)[4])
...
pvals = loadPvals("./analysis_input/ChrAll_AFR.txt")
#selsnps = pvals[which(-log10(pvals$pval) > cutoff),]; write.table(selsnps[, 1], "AFR_significant_snps.txt", quote = F, row.names = F, col.names = F)
#selsnps = pvals[which(-log10(pvals$pval) > cutoff),]; write.table(selsnps[, 1], "AFR_significant_snps.txt", quote = F, row.names = F, col.names = F)
...
plotManhattan(pvals, col = c("darkgrey", paste(c5[1])),title = paste(names(c5)[1], "permuted"), by="none", ylimit = 70)
histLogFreq(-log10(pvals$pval), breaks=1000, col=c5[1])
...
pvals = loadPvals("./analysis_input/ChrAll_EAS.txt")
...
pvals = loadPvals("./analysis_input/ChrAll_EUR.txt")
#sum(-log10(pvals$pval) > cutoff)
plotManhattan(pvals, col = c("darkgrey", paste(c5[4])), title = paste(names(c5)[4], "observed"))
histLogFreq(-log10(pvals$pval), breaks=1000, col=c5[4])

pvals = loadPvals("./analysis_input/chrAll_EUR_Permuted_pval.txt")
plotManhattan(pvals, col = c("darkgrey", paste(c5[4])),title = paste(names(c5)[4], "permuted"))
histLogFreq(-log10(pvals$pval), breaks=1000, col=c5[4])
