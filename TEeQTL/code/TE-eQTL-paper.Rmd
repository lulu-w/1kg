...
mdf = mdf[order(mdf$statistic), 1:(ncol(mdf)-3)]
...
myHeatmap(mdf, scaled = T, hmcol = hmcol)
#dev.off()
...
```{r}
######################
# Heatmap of positively associated PAX5 target

te = "ALU_umary_ALU_7481"
pax5.target = tf.gene.df$target[which(tf.gene.df$symbol == "PAX5")]
assoc = meq.all$all$eqtls
assoc = assoc[which(assoc$snps == te), ]
assoc = attachGeneNames(assoc)
...
assoc = assoc[which(assoc$geneName %in% pax5.target), ]
assoc = assoc[which(assoc$beta > 0), ]
assoc = assoc[order(assoc$pvalue), ]
assoc = assoc[which(assoc$geneName != "PAX5"), ]


# write.csv(assoc, "PAX5-all-positive-associations.csv", quote = F, row.names = F)
# assoc = read.csv("PAX5-all-positive-associations.csv", header = T, stringsAsFactors = F)

genotype = unlist(snps[which(snps$id == te), ])
genotype = genotype[-1]
targetExp = geneExp[which(geneExp$id %in% assoc$gene), ]
targetExp = merge(targetExp, assoc[, c(1, 3)], by.x = "id", by.y = "gene")
rownames(targetExp) = targetExp$geneName
targetExp = targetExp[, -ncol(targetExp)]
targetExp = as.data.frame(t(targetExp[, -1]))
targetExp = split(targetExp, f = as.factor(genotype))
sapply(targetExp, nrow)
targetExp = sapply(targetExp, colMeans)

sel.targetExp = targetExp[which(colSums(apply(targetExp, 1, diff) > 0) == 2), ]
sel.target.pval = assoc[which(assoc$geneName %in% rownames(sel.targetExp)), ]
sel.target.pval = sel.target.pval[match(rownames(sel.targetExp), sel.target.pval$geneName), ]
sel.target.pval = -log10(sel.target.pval$pvalue)
pval.colors = colorRampPalette(c("white", "purple"))(length(sel.target.pval))
pval.colors = pval.colors[order(sel.target.pval)]

pdf("PAX5.positively.assoc.target.pdf", height = 15, width = 5)
myHeatmap(sel.targetExp, RowSideColors=pval.colors)
dev.off()


##################
# Export for SuppTable4 only the PAX5 targets in the heatmap

assoc = read.csv("PAX5-all-positive-associations.csv", header = T, stringsAsFactors = F)
assoc = assoc[which(assoc$geneName %in% rownames(sel.targetExp)), ]
# write.csv(assoc, "PAX5-Heatmap-target-associations.csv", quote = F, row.names = F)
```
...
### Figure 4 Regional SNP-eQTL Association

```{r}
sel.pairs.te <- unique(sel.pairs$snps)
sel.pairs.te <- snploc[which(snploc$snp %in% sel.pairs.te), ]
spacer <- 5e5
te.loci <- data.frame(sel.pairs.te, start = sel.pairs.te$pos-spacer, end = sel.pairs.te$pos+spacer)

te.loci$chr <- gsub("^chr", "", te.loci$chr)
#write.table(te.loci, "./te.loci.txt", quote = F, sep = "\t", row.names = F)
```

```{r}
require(dplyr)
sel.pairs.gene <- sel.pairs[, c("snps", "gene")]
colnames(sel.pairs.gene) <- c("te", "id")
sel.pairs.geneloc <- left_join(sel.pairs.gene, geneloc, by="id")
sel.pairs.geneExp <- left_join(sel.pairs.gene, geneExp, by="id")

for( i in 1:nrow(sel.pairs.gene)){
    geneloc.name <- paste0(sel.pairs.geneloc[i, "te"], ".", sel.pairs.geneloc[i, "id"], ".geneloc")
    geneExp.name <- paste0(sel.pairs.geneExp[i, "te"], ".", sel.pairs.geneExp[i, "id"], ".geneExp" )
    write.table(sel.pairs.geneloc[i, -1], paste0("C:/Users/lwang336/Desktop/workdir/1kg/TEeQTL/regionalAssociation/te.loc/geneloc/", geneloc.name), quote=F, row.names=F, sep = "\t")
    write.table(sel.pairs.geneExp[i, -1], paste0("C:/Users/lwang336/Desktop/workdir/1kg/TEeQTL/regionalAssociation/te.loc/geneExp/", geneExp.name), quote=F, row.names=F, sep = "\t")
}

# sel.geneloc <- sel.pairs.geneloc[!duplicated(sel.pairs.geneloc$id), -1]
# sel.geneExp <- sel.pairs.geneExp[!duplicated(sel.pairs.geneExp$id), -1]
# write.table(sel.geneloc, "C:/Users/lwang336/Desktop/workdir/1kg/TEeQTL/regionalAssociation/te.loc/geneloc.txt", quote = F, row.names = F, sep = "\t")
# write.table(sel.geneExp, "C:/Users/lwang336/Desktop/workdir/1kg/TEeQTL/regionalAssociation/te.loc/geneExp.txt", quote = F, row.names = F, sep ="\t")
```


```{r}

# SNP_file_name = "./regionalAssociation/te.loc/input/genotype.uniq.af0.05.txt"
# snps_location_file_name = "./regionalAssociation/te.loc/input/snploc.uniq.af0.05.txt"
# expression_file_name = "./regionalAssociation/te.loc/input/geneExp.txt"
# gene_location_file_name = "./regionalAssociation/te.loc/input/geneloc.txt"
# 
# covariates_file_name = "./input/covariates.txt"
# 
# p.cut = association.fdr.cut
# fdr.cut = 0.05  
# 
# #Run Matrix eQTL
...
# regional.res <- list(regional.res, loc.meq.eqtls)
...
#     geneloc.name <- paste0(sel.pairs.geneloc[i, "te"], ".", sel.pairs.geneloc[i, "id"], ".geneloc")
#     geneExp.name <- paste0(sel.pairs.geneExp[i, "te"], ".", sel.pairs.geneExp[i, "id"], ".geneExp" )
#     genotype.name <- paste0(sel.pairs.geneloc[i, "te"], ".genotype")
#     snploc.name <- paste0(sel.pairs.geneloc[i, "te"], ".snploc")
#     loc.te.gene <- paste0(sel.pairs.geneloc[i, "te"], ".", sel.pairs.geneloc[i, "id"])

##########################
# Regional association for ALU_umary_ALU_5788 and ENSG00000162924

geneloc.name = "ALU_umary_ALU_5788.ENSG00000162924.geneloc"
geneExp.name = "ALU_umary_ALU_5788.ENSG00000162924.geneExp"
snploc.name = "ALU_umary_ALU_5788.snploc"
genotype.name = "ALU_umary_ALU_5788.genotype"
  
SNP_file_name = paste0("./regionalAssociation/te.loc/genotype/", genotype.name)
snps_location_file_name = paste0("./regionalAssociation/te.loc/snploc/", snploc.name)
expression_file_name =  paste0("./regionalAssociation/te.loc/geneExp/", geneExp.name)
gene_location_file_name = paste0("./regionalAssociation/te.loc/geneloc/", geneloc.name)
#expression_file_name = "./input/geneExp.txt"
#gene_location_file_name = "./input/geneloc.txt"
...
write.table(loc.meq.eqtls, "REL.eqtls.txt", row.names = F, quote = F)

    # 
    # regional.res[[i]] = loc.meq.eqtls
    # 
# }

# regional.res = lapply(regional.res, function(x) x = x[order(x$pvalue),])
# regional.res.best = lapply(regional.res, head, n=1)
# regional.res.best = do.call(rbind, regional.res.best)
# regional.res.te = regional.res.best[grep("ALU|L1|SVA", regional.res.best$snps, perl = T),]
# regional.res.te = attachGeneNames(regional.res.te)
#write.csv(regional.res.te, "regional_best_TEeQTL.csv", quote = F, row.names = F)


source("./regionalAssociation/make.fancy.locus.plot.R")

pvals = read.table("REL.eqtls.txt", header = T, stringsAsFactors = F)
pvals = pvals[which(pvals$POS > 6200000 & pvals$POS < 6400000), ]
# pvals = pvals[order(pvals$POS), ]
# pvals = pvals[c(round(nrow(pvals)/3):round(2*nrow(pvals)/3)), ]
title = "REL association with Alu-5788 locus"


make.fancy.locus.plot(snp = "ALU_umary_ALU_5788", title = title, chr = as.character(pvals$CHR[1]), data = pvals, nlp.range = 9, best.pval = pvals[which(pvals$SNP == "ALU_umary_ALU_5788"), 4], lim.strong = 1e-6, lim.mod = 1e-4, lim.weak = 1e-2)
...
```{r}
# regional.res.te = read.csv("regional_best_TEeQTL.csv", header = T)
ploteqtl(regional.res.te[9,], ylim = c(-2, 12))

ploteqtl(regional.res.te[7,], ylim = c(10, 81))
# export 5x3


# pdf("test.pdf")
# lapply(1:13, function(d){ ploteqtl(regional.res.te[d,])})
# dev.off()
...
```{r}
##########################
# Regional association for ALU_umary_ALU_7481 and PAX5 (ENSG00000196092)

geneloc.name = "ALU_umary_ALU_7481.ENSG00000196092.geneloc"
geneExp.name = "ALU_umary_ALU_7481.ENSG00000196092.geneExp"
snploc.name = "ALU_umary_ALU_7481.snploc"
genotype.name = "ALU_umary_ALU_7481.genotype"
...
## Apply AF cutoff of 0.05
snps = fread(SNP_file_name)
AF = rowSums(snps[, 2:ncol(snps), with = F])
sel = AF > 0.05*2*(ncol(snps)-1)
snps = snps[sel, ]
SNP_file_name = gsub("genotype", "genotype.af0.05", SNP_file_name)
write.table(snps, SNP_file_name, quote = F, row.names = F, sep = "\t")
...
loc.meq.eqtls = loc.meq.eqtls[order(loc.meq.eqtls$PVAL), ]
loc.meq.eqtls = loc.meq.eqtls[!duplicated(loc.meq.eqtls$SNP), ]
write.table(loc.meq.eqtls, "PAX5.eqtls.txt", row.names = F, quote = F)


source("./regionalAssociation/make.fancy.locus.plot.R")

pvals = read.table("PAX5.eqtls.txt", header = T, stringsAsFactors = F)
#pvals = pvals[which(pvals$POS > 6200000 & pvals$POS < 6400000), ]
title = "PAX5 association with Alu-7481 locus"


make.fancy.locus.plot(snp = "ALU_umary_ALU_7481", title = title, chr = as.character(pvals$CHR[1]), data = pvals, nlp.range = 12, best.pval = pvals[which(pvals$SNP == "ALU_umary_ALU_7481"), 4], lim.strong = 1e-6, lim.mod = 1e-4, lim.weak = 1e-2)
...
te.name = regional.res.te[7, 1] #ALU_umary_ALU_10841
gene.id = regional.res.te[7, 3] #PSD4

SNP_file_name = paste0("./regionalAssociation/te.loc/genotype/", te.name, ".genotype")
snps_location_file_name = paste0("./regionalAssociation/te.loc/snploc/", te.name, ".snploc")
expression_file_name =  paste0("./regionalAssociation/te.loc/geneExp/", te.name, ".", gene.id, ".geneExp")
gene_location_file_name = paste0("./regionalAssociation/te.loc/geneloc/", te.name, ".", gene.id, ".geneloc")
covariates_file_name = "./input/covariates.txt"
p.cut = 1
fdr.cut = 0.05  
#Run Matrix eQTL
...
loc.meq.eqtls = loc.meq.eqtls[order(loc.meq.eqtls$PVAL), ]
loc.meq.eqtls = loc.meq.eqtls[!duplicated(loc.meq.eqtls$SNP), ]
#write.table(loc.meq.eqtls, "PSD4.eqtls.txt", row.names = F, quote = F)



source("./regionalAssociation/make.fancy.locus.plot.R")

pvals = read.table("PSD4.eqtls.txt", header = T, stringsAsFactors = F)
pvals = pvals[which(pvals$POS > 81300000 & pvals$POS < 81500000), ]
title = "PSD4 association with Alu-10841 locus"


make.fancy.locus.plot(snp = "ALU_umary_ALU_10841", title = title, chr = as.character(pvals$CHR[1]), data = pvals, nlp.range = 9, best.pval = pvals[which(pvals$SNP == "ALU_umary_ALU_10841"), 4], lim.strong = 1e-6, lim.mod = 1e-4, lim.weak = 1e-2)
# export 4x6
...
# loc1.meq.eqtls = loc1.meq.eqtls[order(loc1.meq.eqtls$pvalue), ]
# locus = loc1.meq.eqtls[, c("snps", "pvalue")]
# locus = locus[order(locus$snps, locus$pvalue), ]
# locus = locus[!duplicated(locus$snps), ]
# 
# regsnpsloc <- read.table(snps_location_file_name, header = T, stringsAsFactors = F)
# 
# locus = merge(locus, regsnpsloc, by.x = "snps", by.y = "snp")
# locus = locus[, c(1, 4, 2)]
# names(locus) = c("SNP", "POS", "PVAL")
# rownames(locus) = locus[, 1]
# locus$TYPE="typed"
# 
# source("./regionalAssociation/make.fancy.locus.plot.R")
# 
# te <- meq.all.mdf[which(meq.all.mdf$snps=="ALU_umary_ALU_5072" & meq.all.mdf$name=="HLA-DRB5"), ]
# teloc <- snploc[which(snploc$snp=="ALU_umary_ALU_5072"), "pos"]
# te <- data.frame(SNP=te$snps, POS=teloc, PVAL=te$pvalue, TYPE="typed")
# rownames(te) = te$SNP
# 
# locus1 = locus[which(locus$POS > 32489834 & locus$POS < 32689834), ]
# locus1 = rbind(locus1, te)

# locus1 = read.table("./regionalAssociation/DGI_chr9_hit.txt", header = T)
# 
# make.fancy.locus.plot("rs10811661", "HLA-DRB5 Locus", "6", locus1, 8, 3.57E-05)
# 
# make.fancy.locus.plot("rs147014445", "", "9", locus1, 8, 3.533956e-17)
```
