...
rm -r genotype; mkdir genotype; rm -r multiallelic; mkdir multiallelic; rm -r snploc; mkdir snploc;

### 445 Individuals
../code/te_loci.sh All.te.loci.txt
```
...
# # All
# 
# res = apply(te.loci[['All']], 1, function(d, snploc, snps){
#   te = d[1]
#   chr = paste0("chr", d[2])
...
#   write.table(add.teloc, paste0("./regionalAssociation/All.te.loc/snploc/", te, ".snploc"), col.names = F, row.names = F, sep = "\t", append = T, quote = F)
#   add.tegenotype = snps[which(snps$id %in% add.teloc$snp), ]
#   write.table(add.tegenotype, paste0("./regionalAssociation/All.te.loc/genotype/", te, ".genotype"), col.names = F, row.names = F, sep = "\t", append = T, quote = F)
...
all.expression = geneExp[which(geneExp$id %in% meqs.filtered.best$All$gene ), ]
all.geneloc = geneloc[which(geneloc$id %in% all.expression$id), ]
for(i in 1:nrow(all.expression)){
  write.table(all.expression[i, ], paste0("./regionalAssociation/All.te.loc/geneExp/", all.expression[i, 1], ".geneExp"), row.names = F, quote = F, sep = "\t")
  write.table(all.geneloc[i, ], paste0("./regionalAssociation/All.te.loc/geneloc/", all.geneloc[i, 1], ".geneloc"), row.names = F, quote = F, sep = "\t")
}
```
