...
ploteqtl <- function(x, show = "both", by = "group", ylim = NULL){
  mdf = checkExp(x)
  ...
  if(by == "genotype"){
    ...
    g <- ggplot(mdf, aes(genotype, expression)) + 
      geom_point(size = 2, alpha = 1/2, position = position_jitter(width = 0.2), aes(col = factor(group))) + 
      geom_boxplot(alpha = 1/2, outlier.size = 0) + 
      labs(title = paste(sid, "vs", gname)) + 
      xlab(paste("P=", pval, "FDR=", fdr, " Chisq.P=", chisq.p)) + 
      theme_bw() + 
      theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) + 
      scale_color_manual(values = c("#F9A41B", "#3954A3")) +
      guides(color = F)
    ...
    slices = slices/colSums(slices)
    par(mfrow=c(1,3))
    for(i in 1:nrow(slices)){
      pie(slices[i, ], col = c("#F9A41B", "#3954A3"), labels = "", border = NA)
    }   
  }else if(by == "none"){
