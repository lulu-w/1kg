...
### Population Expression Structure
...
#lapply(1:top.n, function(x) ploteqtl(sel.pairs[x, ], by = "group"))
#dev.off()

sel.pairs = split(sel.pairs, f = sel.pairs$snps)
sel.pairs.hm <- which(sapply(sel.pairs, nrow) > 5 )

pdf("population.specific.TE-gene.pdf", height = 5, width = 5)
res3 <- lapply(sel.pairs[sel.pairs.hm], extractExpression)
...
for (i in 1:chrNum){ 
  ...
  if (i %% 2 == 0) te.assoc.track$group[ndx] = "even"
...
ggplot(te.assoc.track, aes(x = pos, y = -log10(pvalue))) + geom_point(aes(shape = factor(snpgroup), color = factor(chr, levels = paste0("chr", 1:chrNum)))) + scale_color_manual(values = rep(c("black", "red"), chrNum/2))
...
```{r circosPlotLinks}
consistent.tar = read.csv("./input/TE-consistent-targets-from-heatmap.csv", header = T)
