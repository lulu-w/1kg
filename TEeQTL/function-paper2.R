...
myMEQQplot = function(pvalfile, permutedfile, n_probs=100000, p_cut=0.01, col="black", title="", ylimit = NULL){
  require(data.table)
  pvals = fread(pvalfile, header = T)
  ppvals = fread(permutedfile, header = T)
  ...
  ################
  # By definition, lambda = median(observed_chisq)/qchisq(0.5,1). However, larger chisq
  # actually corresponds to lower p-value. So here I use median(observed_p)/median(expected_p)
  lambda = 1/lambda
  ...
  #par(cex=0.7)
  tick_space=10
  if(is.null(ylimit)) { ylimit = (max_obs_q %/% tick_space + 1)*tick_space }
  plot(exp_q, obs_q, col=col, xlab=expression(-log[10]*(Expected)), ylab=expression(-log[10]*(Observed)), main=pop_name, ylim=c(2, ylimit), xlim=c(min_exp_q, max_exp_q+1), axes=F, pch = 20)
  points(exp_q, pobs_q, col="darkgrey", pch = 20)
  lines(exp_q, obs_q, col = col)
  lines(exp_q, pobs_q, col="darkgrey")
  ...
  axis(side=2, pos=min_obs_q, at=c(min_obs_q, seq((min_obs_q %/% tick_space + 1)*tick_space, ylimit, by=tick_space)))
  abline(0,1)
...
ploteqtl <- function(x, show = "both", by = "group"){
  mdf = checkExp(x)
  
  sid = as.character(x$snps)
  gid = as.character(x$gene)  
  gname = ifelse(is.null(as.character(x$geneName)), gid, as.character(x$geneName))
  pval = format(as.numeric(x$pvalue), digits = 3, scientific = T)
  chisq.p = format(as.numeric(x$chisq.p), digits = 3, scientific = T)
  fdr = format(as.numeric(x$FDR), digits = 3, scientific = T)
  
  # if(show == "YRI"){
  #   mdf = mdf[which(mdf$group == "YRI"), ]
  # }else if(show == "EUR"){
  #   mdf = mdf[which(mdf$group == "EUR"), ]
  # }
  # 
  # ggplot(mdf, aes(genotype, expression)) + 
  #   geom_point(size = 3.5, alpha = 1/2, position = position_jitter(width = 0.2)) + 
  #   geom_boxplot(alpha = 1/2, outlier.size = NA) + 
  #   labs(title = paste(sid, "vs", gname)) + 
  #   xlab(paste("P=", pval, "FDR=", fdr, " Chisq.P=", chisq.p)) + 
  #   theme_bw() + 
  ...
  if(by == "genotype"){
    ...
    g <- ggplot(mdf, aes(genotype, expression)) + 
      geom_point(size = 3.5, alpha = 1/2, position = position_jitter(width = 0.2), aes(col = factor(group))) + 
      geom_boxplot(alpha = 1/2, outlier.size = NA) + 
      labs(title = paste(sid, "vs", gname)) + 
      xlab(paste("P=", pval, "FDR=", fdr, " Chisq.P=", chisq.p)) + 
      theme_bw() + 
...
attachGeneNames = function(d){
  require(data.table)
  
  # ############################################
  # # Run once to generate the ensg2name file
  # 
  # # UCSC hg19 table "ensemblToGeneName"
  # ens2name = fread("../input/ENSEMBL_to_GeneName.txt")
  # setnames(ens2name, c("ENST", "name"))
  # 
  # # UCSC hg19 table "ensGtp"
  # ensGTP = fread("../input/ENSEMBL-ensGTP.txt")
  # ensGTP = ensGTP[, 1:2, with = F]
  # setnames(ensGTP, c("ENSG", "ENST"))
  # 
  # # ENSG to GeneName mapping
  # ensg2name = merge(ens2name, ensGTP, by="ENST")
  # ensg2name = ensg2name[, c(3, 2), with = F]
  # u = which(!duplicated(paste0(ensg2name$ENSG, "", ensg2name$name)))
  # ensg2name = ensg2name[u, ]
  # setnames(ensg2name, c("gene", "geneName"))
  # write.table(ensg2name, "./data/ENSG2name.txt", quote = F, row.names = F)
  # # #############################################
  ensg2name = fread("./input/ENSG2name.txt")
  
  d = merge(d, ensg2name, by = "gene", all.x = T)
  d$geneName = ifelse(is.na(d$geneName), "NA", d$geneName)
  d = d[order(d$pvalue), ]
  return(d)
}